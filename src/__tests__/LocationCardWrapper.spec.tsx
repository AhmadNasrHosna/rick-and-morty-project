...
test('Renders location card component', async () => {
  ...
  const locationCardWrapper = await screen.findByTestId('location-card-wrapper');
...
test('Renders location card wrapper name', async () => {
  ...
  const locationCardWrapperName = await screen.findByTestId('location-card-wrapper-name');
...
test('Renders location card wrapper explanation with correct featured residents', async () => {
  ...
  expect(locationCardWrapperExplanation).toHaveTextContent(fakeData.residents.length.toString());
});
...
test('After load more onClick', async () => {
  await act(async () => {
    ...
    await waitFor(() => [
      ...
      expect(screen.getAllByTestId('character-feature-name').length).toEqual(19),
    ]);
  });
});
